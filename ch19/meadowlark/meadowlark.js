...
Dealer.find({}, function(err, dealers){
    ...
    // 난곡동, 청룡동, 미성동 등등에 대해 같은 코드 반복...

    new Dealer({
	name: '남현동주민센터',
	address1: '관악구 남현동 602-253',
	city: '서울',
	country: 'KO',
	phone: '02-6291-3049',
	ative: true,
...
function geocodeDealer(dealer){
    ...
    if(addr===dealer.geocodedAddress) return;        // 이미 지오코딩됨
	...
	if(Date.now() > dealerCache.geocodeCount + 24 * 60 * 60 + 1000){
	    ...
	    dealerCahe.geocodeCount = 0;
	} else {
	    // 사용 제한을 초과했으므로 지금은 할 수 없음
	    return;
	}
    }
